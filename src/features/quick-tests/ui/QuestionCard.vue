...
<template>
  <div class="w-full rounded-xl border bg-card p-5 text-card-foreground shadow-sm">
    ...
    <div v-if="question.kind !== 'text'" class="mt-4 grid gap-2 max-w-4xl">
      ...
      >
        <span
          class="mt-1 grid h-6 w-6 place-content-center rounded-full border text-xs font-semibold"
          :class="[
    ...
    >
      Modo treino: resposta correta {{ question.kind === "multiple_choice" ? "inclui" : "Nao inclui" }}:
